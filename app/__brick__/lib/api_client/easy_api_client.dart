...
import 'package:{{packageName}}/core/api_client/map_dio_errror.dart';
import 'package:{{packageName}}/core/api_client/result.dart';
import 'package:{{packageName}}/core/api_client/success_data.dart';
...
class EasyApiClient {
  ...
  Future<Result<T>> request<T>(
    ...
    try {
      ...
      // DioException.message is often null for thrown/hand-built errors; log error + body.
      log(
        '$method $path DioException: ${e.message ?? e.error}'
        '${e.response?.data != null ? ' | data: ${e.response!.data}' : ''}',
      );
  ...
  // TODO: проверяет статус ответа на уровне приложения
  void _checkAppLevelStatus(Response response) {
    final data = response.data;
    if (data is! Map) return;

    final rawStatusCode = data['statusCode'];
    final statusCode = rawStatusCode is num
        ? rawStatusCode.toInt()
        : int.tryParse(rawStatusCode?.toString() ?? '');
    ...
    throw DioException(
      ...
      type: DioExceptionType.badResponse,
      error: 'Application-level error with statusCode: $statusCode',
    );
  }
